...
public class ForceStasis implements Ability {
    ...
    public ForceStasis(AbilityConfigManager configManager, ForcePlugin plugin) {
    ...
    public String getDescription() {
        return "Freezes a single target in place.";
    }
    ...
    public double getEnergyCost(int level) {
        return configManager.getDoubleValue(getID(), level, "energy-cost", 20.0);
    }

    @Override
    public double getCooldown(int level) {
        return configManager.getDoubleValue(getID(), level, "cooldown", 15.0);
    }
