...
public class ForceSerenity implements Ability {
    ...
    public void execute(Player player, ForceUser forceUser) {
        ...
        player.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, duration, 200)); // Root in place
        player.addPotionEffect(new PotionEffect(PotionEffectType.RESISTANCE, duration, 2)); // High resistance
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (double angle = 0; angle < 2 * Math.PI; angle += Math.PI / 16) {
                    ...
                    player.getWorld().spawnParticle(Particle.HAPPY_VILLAGER, new Location(player.getWorld(), x, center.getY() + 0.5, z), 1, 0, 0, 0, 0);
                }
                ticks++;
            }
