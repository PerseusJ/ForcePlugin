...
public class ForceHeal implements Ability {
    ...
    // --- THE FIX: Corrected PotionEffectType names ---
    private static final List<PotionEffectType> DEBUFFS = Arrays.asList(
            ...
            PotionEffectType.NAUSEA, PotionEffectType.HUNGER,
    ...
    public String getDescription() {
        return "Restores health and cleanses negative effects.";
    }
    ...
    public double getCooldown(int level) {
        return configManager.getDoubleValue(getID(), level, "cooldown", 12.0);
    }
    ...
    public void execute(Player player, ForceUser forceUser) {
        ...
        if (player.getHealth() >= player.getMaxHealth() && !hasDebuff) {
            return;
        }
